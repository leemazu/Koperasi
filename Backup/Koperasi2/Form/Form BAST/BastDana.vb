...
Public Class BastDana
    ...
    Private Sub gv1_CellContentDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gv1.CellContentDoubleClick
        ...
        StrSQL = ""
        StrSQL = "SELECT Bunga,BungaPengali,EffectiveInterestP,Pokok FROM Application where ApplicationID='" & CStr(antisqli(txtAplikasi.Text)) & "'"
        RunSQL(StrSQL, 1)
    ...
    Private Sub ContractDetail_SAV()
        ...
        Dim tanggalJatuhTempo As Integer
        Dim bulanJatuhTempo As Integer
        Dim tahunJatuhTempo As Integer
        ...
        Dim Op As Double
        Dim Oi As Double
        tahun = Microsoft.VisualBasic.DateAndTime.Year(dtAngsuran.Value)
        bulan = Microsoft.VisualBasic.DateAndTime.Month(dtAngsuran.Value)
        tanggalJatuhTempo = Microsoft.VisualBasic.DateAndTime.Day(dtAngsuran.Value)
        tahunJatuhTempo = Microsoft.VisualBasic.DateAndTime.Year(dtAngsuran.Value)
        bulanJatuhTempo = Microsoft.VisualBasic.DateAndTime.Month(dtAngsuran.Value)
        BungaPengali = CDbl(txtBungaPengali.Text)
        pokok = CDbl(txtPokok.Text)
        Bunga = CDbl(txtBunga.Text)
        EffectiveInterestP = CDbl(txtEffectiveInterest.Text)
        Dim tenor As Integer = CInt(txtTenor.Text)


        For index As Integer = 0 To tenor - 2
            bulanJatuhTempo = bulan + index
        ...
        lastTenor(tempIndex, tempJatuhTempo, Sp, Si, Op, Oi, pokok, Bunga)
    ...
    Private Sub gv1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gv1.CellContentClick
        ...
        txtKontrak.Text = CStr(AutoNumberContract(CStr(txtAplikasi.Text), "01"))
        StrSQL = ""
        StrSQL = "SELECT BungaPengali,Pokok FROM Application where ApplicationID='" & CStr(antisqli(txtAplikasi.Text)) & "'"
        RunSQL(StrSQL, 1)
        txtBungaPengali.Text = CStr(dt.Rows(0)("BungaPengali"))
        txtPokok.Text = CStr(dt.Rows(0)("Pokok"))
    End Sub

    Private Sub lastTenor(ByVal index As Integer, ByVal JatuhTempo As String, ByVal sp As Double, ByVal si As Double, ByVal op As Double, ByVal oi As Double, ByVal pokok As Double, ByVal bunga As Double)
        ...
        tanggal = Microsoft.VisualBasic.DateAndTime.Day(CDate(JatuhTempo))
